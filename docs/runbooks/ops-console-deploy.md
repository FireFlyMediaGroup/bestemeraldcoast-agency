...
5. **Build & Output settings** (Settings → Build and Deployment). These are **not** the Next.js defaults — a plain `next build` from `apps/ops-console` fails on Vercel for two verified reasons, so override:
   - **Build Command** (Override ON):
     ...
     Why: (a) `@bec/db` / `@bec/config` / `@bec/logger` / `@bec/rate-limit` resolve their *runtime* entry to compiled `dist/` (`exports.default → ./dist/index.js`); nothing builds those on a clean Vercel checkout, so `next build` dies with `Module not found: Can't resolve '@bec/db'`. Turbo's `build` task `dependsOn: ["^build"]` builds those deps' `dist/` first. (b) The `apps/ops-console` `build` script is `next build --webpack` — **Turbopack** (Next 16's default `next build`) **deadlocks** for this app (every thread parked in `uv_cond_wait`, 0% CPU — a hang, *not* OOM; more RAM does not help). The webpack path is verified working. Because the package script carries `--webpack`, the turbo filter command above needs no extra flags. (Equivalent explicit form if you must bypass turbo: `cd ../.. && pnpm turbo run build --filter=@bec/ops-console^... && cd apps/ops-console && next build --webpack`.)
     - **⚠️ DO NOT use a hand-written `--filter=@bec/db --filter=@bec/config --filter=@bec/logger` list** — that "explicit" form was sometimes used in past Vercel project settings, and it silently breaks every time a new workspace dep is added (e.g. `@bec/rate-limit` in Commit 2.11 → ops-console build failed `Module not found: Can't resolve '@bec/rate-limit'` because the explicit list didn't pick it up). The `--filter=@bec/ops-console` form follows the dependency graph automatically and is the only correct one. If the Vercel project shows the explicit form, replace it with the auto-pickup form before the next deploy.
