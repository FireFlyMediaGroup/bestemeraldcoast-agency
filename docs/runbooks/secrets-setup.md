...
| `POSTHOG_API_KEY` (server) | [PostHog](https://us.posthog.com) | Project → Settings → Project API Keys → use the **Project API key** (`phc_...`) for server-side event capture. (Personal API keys exist too, but they're for management endpoints and are over-privileged for ingestion — don't use them here.) | `PostHog server API key — bec-prod` | ADR-011 |
