...
| 7b — Enable auto-merge | `gh pr merge --auto --squash --delete-branch`. Auto-merge will not fire until **all required checks** pass — CI **and** CodeRabbit |
| 7c — Wait for CodeRabbit's first review | poll `gh pr view <PR> --json reviews --jq '.reviews \| map(select(.author.login == "coderabbitai[bot]")) \| last'` until a review object exists. Typical: 1–3 min. Timeout: 10 min — if no review, check the CodeRabbit App is installed on the repo |
| 7d — Triage CodeRabbit comments | Read all comments via `gh pr view <PR> --comments` (or `gh api repos/FireFlyMediaGroup/bestemeraldcoast-agency/pulls/<N>/comments`). For each: **fix** (push to branch → CodeRabbit auto re-reviews → loop back to 7c) or **reply with rationale** via `gh pr comment <PR> --body "@coderabbitai <rationale>"`. Never silently dismiss. The Reviewer agent owns this step |
| 7e — Confirm APPROVED | poll the latest CodeRabbit review until `state == "APPROVED"`. **Hard gate**: do not proceed without APPROVED. If CodeRabbit insists on CHANGES_REQUESTED for something genuinely out-of-scope, escalate — open an ADR amendment in `docs/dev/adr-log.md` rather than overriding |
| 7f — Wait for merge | poll `gh pr view <PR> --json state,mergedAt,mergeCommit` until `state == "MERGED"`; capture `mergeCommit.oid` |
| 8 — Ship (`/ship-task`) | append entry to `task-log.md` with PR URL + merge SHA + CodeRabbit final state + comment-resolution count; rewrite `next-step.md` for the next commit |
