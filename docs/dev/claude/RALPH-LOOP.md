...
| 7b — Wait for review before enabling auto-merge | **Pre-Pass-2** (no required status checks yet): do NOT enable auto-merge immediately. Pass-1 has `required_approving_review_count: 0` and no required checks, so auto-merge fires within seconds — faster than CodeRabbit's review webhook. Poll first (step 7c), wait for `APPROVED`, then `gh pr merge <PR> --auto --squash --delete-branch`. **Post-Pass-2** (when CI + CodeRabbit are required checks per Commit 0.6+ → Pass-2 protection): the rule reverses — enable auto-merge immediately and let the required checks gate the merge naturally |
| 7c — Wait for CodeRabbit's first review | poll `gh pr view <PR> --json reviews --jq '.reviews \| map(select(.author.login == "coderabbitai")) \| last'` until a review object exists. Bot login is **`coderabbitai`** (no `[bot]` suffix — verified on this repo). Typical: 1–3 min. Timeout: 10 min. If the PR is a draft and `.coderabbit.yaml` has `auto_review.drafts: false`, no auto-review will land — manually trigger with `gh pr comment <PR> --body "@coderabbitai review"` |
| 7d — Triage AI-reviewer comments | Read all comments via `gh pr view <PR> --comments` (or `gh api repos/FireFlyMediaGroup/bestemeraldcoast-agency/pulls/<N>/comments`). **Treat every active AI reviewer as gating.** This repo has `coderabbitai` and `cubic-dev-ai` installed; both can post `CHANGES_REQUESTED`-style findings and both must be cleared before merge. For each finding: **fix** (push to branch → on a non-draft PR, CodeRabbit auto re-reviews; on a draft PR with `auto_review.drafts: false`, comment `@coderabbitai review` after each push → loop back to 7c) or **reply with rationale** via `gh pr comment <PR> --body "@coderabbitai <rationale>"`. Never silently dismiss. The Reviewer agent owns this step |
| 7e — Confirm APPROVED + re-fetch immediately before merge | Poll the latest CodeRabbit review until `state == "APPROVED"`. **Hard gate**: do not proceed without APPROVED. Then — **immediately before calling `gh pr merge`** — re-fetch `gh pr view <PR> --json reviewDecision` one more time. CodeRabbit may post a fresh `CHANGES_REQUESTED` between the earlier APPROVED and the merge call (this happened on PR #4 during Commit 0.4-prep). Cached state lies. If CodeRabbit insists on CHANGES_REQUESTED for something genuinely out-of-scope, escalate — open an ADR amendment in `docs/dev/adr-log.md` rather than overriding |
