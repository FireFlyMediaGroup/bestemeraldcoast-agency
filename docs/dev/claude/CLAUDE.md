...
- Never push to `main` directly. Every change lands via a PR opened from a `phase-<N>/commit-<N>.<M>-<slug>` branch and squash-merges after the **required CI checks** (`lint`, `type-check`, `unit-tests` — Pass-2 branch protection) are green. See `RALPH-LOOP.md` § Git Discipline.
- **CodeRabbit is advisory, not a merge gate** (amended 2026-05-16, adr-log). Still never dismiss a CodeRabbit comment without addressing it: each posted finding is fixed (push to branch) or replied-to with rationale (`gh pr comment`). But CodeRabbit's review *state* (`APPROVED` / `CHANGES_REQUESTED`) is **not** a merge precondition, and a stalled / rate-limited / never-posted CodeRabbit review does **not** block merge. The merge gate is: required CI checks green **and** cubic clean **and** every *posted* CodeRabbit finding triaged (fixed or replied). Rationale: across PRs #8/#9/#11/#15/#16 CodeRabbit repeatedly stalled, rate-limited, and re-flagged already-fixed code via stale incremental anchoring — blocking on it stalled the loop with no quality signal. CI + cubic + the human/agent finding-triage are the reliable gate. If CodeRabbit insists on something genuinely out-of-scope, still escalate via an `docs/dev/adr-log.md` amendment rather than silently overriding.
